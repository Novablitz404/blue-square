...
export function ActivityTracker({ activeTab, setActiveTab }: ActivityTrackerProps) {
  ...
  const getLevel = (points: number) => {
      if (points >= 1000) return { level: "Diamond Hands", color: "text-purple-500", nextLevel: null, progress: 100 };
  if (points >= 500) return { level: "Whale", color: "text-blue-500", nextLevel: "Diamond Hands", progress: Math.min(((points - 500) / 500) * 100, 100) };
  if (points >= 200) return { level: "DeFi Master", color: "text-yellow-500", nextLevel: "Whale", progress: Math.min(((points - 200) / 300) * 100, 100) };
  if (points >= 100) return { level: "Crypto Native", color: "text-gray-400", nextLevel: "DeFi Master", progress: Math.min(((points - 100) / 100) * 100, 100) };
  if (points >= 50) return { level: "HODLer", color: "text-orange-500", nextLevel: "Crypto Native", progress: Math.min(((points - 50) / 50) * 100, 100) };
  return { level: "Newbie", color: "text-green-500", nextLevel: "HODLer", progress: Math.min((points / 50) * 100, 100) };
  };
